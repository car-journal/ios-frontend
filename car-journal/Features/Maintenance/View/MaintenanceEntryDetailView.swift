...
struct MaintenanceEntryDetailView: View {
    ...
    var body: some View {
        ZStack {
            ...
            if viewModel.isLoadingEntry && viewModel.currentEntry == nil {
        ...
        .task {
            // Fetch fresh data by ID (includes odometer_reading)
            await viewModel.findByID(entryID: entry.id.uuidString)
            await viewModel.loadCategories()
            // Set category name after categories are loaded
            if let cat = viewModel.categories.first(where: { $0.id.uuidString == viewModel.form.categoryId }) {
